  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-zinc-100 leading-tight">User Management</h1>
          <p className="text-zinc-400 text-sm">Manage users, roles, and permissions</p>
        </div>
        <div className="flex items-center gap-3">
          <button className="flex items-center gap-2 px-4 py-2.5 bg-zinc-900/40 backdrop-blur-xl border border-blue-200 text-blue-600 hover:bg-blue-50 rounded-xl text-sm font-semibold transition-colors shadow-sm">
      ...
      <div className="bg-zinc-900/40 backdrop-blur-xl rounded-2xl border border-zinc-800/60 shadow-sm overflow-x-auto">
        <table className="w-full text-sm text-left">
          <thead className="text-xs text-zinc-100 font-bold border-b border-zinc-800/60 bg-zinc-950/50/50">
            <tr>
          ...
          <tbody className="divide-y divide-slate-100">
            {users.map((user, i) => (
              <tr key={i} className="hover:bg-zinc-950/50/50 transition-colors group">
                ...
                {/* Role */}
                <td className="px-6 py-4">
                  <span className="font-bold text-zinc-100">{user.role}</span>
                </td>
                ...
                {/* Dates */}
                <td className="px-6 py-4">
                  <div className="text-zinc-400 font-medium whitespace-pre-wrap">{user.joinDate.replace(' ', '\n')}</div>
                </td>
                <td className="px-6 py-4">
                  <div className="text-zinc-400 font-medium whitespace-pre-wrap">{user.lastUpdate.replace(' ', '\n')}</div>
                </td>
