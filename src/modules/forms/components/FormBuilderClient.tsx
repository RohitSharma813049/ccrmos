...
export default function FormBuilderClient({ formId }: { formId: string }) {
  ...
  if (loading) return <div className="p-8 text-center text-zinc-400">Loading builder...</div>;
    ...
    <div className="space-y-6 fade-in pb-20 max-w-4xl mx-auto p-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 space-y-4">
          ...
          {/* Header Editor */}
          <div className="bg-zinc-900/40 backdrop-blur-xl p-6 rounded-2xl shadow-sm border border-zinc-700/50 border-t-4 border-t-indigo-500">
            <input 
              type="text" 
              value={form.title}
            ...
            <textarea 
              ...
              onBlur={() => saveForm()}
              rows={2}
              className="w-full mt-3 text-zinc-400 border-none outline-none focus:ring-0 px-0 bg-transparent resize-none placeholder-gray-400"
              ...
              {(provided) => (
                <div {...provided.droppableProps} ref={provided.innerRef} className="space-y-4">
                  {(!form.fields || form.fields.length === 0) ? (
                    <div className="bg-zinc-900/40 backdrop-blur-xl/50 border-2 border-dashed border-zinc-700/50 rounded-2xl p-12 text-center text-zinc-400">
                                      ...
                                      {field.type === 'textarea' ? (
                                        ...
                                        <div className="space-y-2">
                                          {(field.options || []).slice(0,3).map((opt: string, i: number) => (
                                            <div key={i} className="flex items-center gap-2"><div className="w-4 h-4 border border-zinc-700/50 rounded-full" /><span className="text-sm text-zinc-400">{opt}</span></div>
                                          ))}
                                        </div>
                                      ) : (
                  ...
                  <button 
                    ...
                    className="w-full py-4 border-2 border-dashed border-zinc-700/50 rounded-2xl text-zinc-400 font-semibold hover:border-indigo-400 hover:text-indigo-600 hover:bg-indigo-50/50 transition-all flex items-center justify-center gap-2"
                  >
        ...
        <div className="lg:col-span-4">
          <div className="bg-zinc-900/40 backdrop-blur-xl p-6 rounded-2xl shadow-sm border border-zinc-700/50 sticky top-6">
            <h3 className="font-bold text-zinc-100 mb-4 border-b pb-2">Form Settings</h3>
            
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-1.5">Submit Button Text</label>
                <input 
                  type="text" 
                  value={form.submitButtonText || "Submit"}
                  onChange={(e) => setForm({...form, submitButtonText: e.target.value})}
                  onBlur={() => saveForm()}
                  className="w-full px-3 py-2 text-sm bg-zinc-950/50 border border-zinc-700/50 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
                />
              </div>
              ...
              <div>
                <label className="flex items-center gap-2 cursor-pointer mt-4">
                  <input 
                    ...
                    className="w-4 h-4 text-indigo-600 border-zinc-700/50 rounded focus:ring-indigo-500"
                  />
                  <span className="text-sm font-medium text-zinc-300">Notify me on new submissions</span>
                </label>
              </div>

              <div>
                <label className="block text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-1.5">Linked Project</label>
                <select 
                  value={form.projectId || ""}
                  onChange={(e) => {
                    const updated = {...form, projectId: e.target.value || null};
                    setForm(updated);
                    saveForm(updated);
                  }}
                  className="w-full px-3 py-2 text-sm bg-zinc-950/50 border border-zinc-700/50 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
              ...
              <div>
                <label className="block text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-1.5">Success Message</label>
                <textarea 
                  value={form.successMessage || "Thank you for your submission!"}
                  onChange={(e) => setForm({...form, successMessage: e.target.value})}
                  onBlur={() => saveForm()}
                  rows={3}
                  className="w-full px-3 py-2 text-sm bg-zinc-950/50 border border-zinc-700/50 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 resize-none"
                />
              </div>
            </div>
          ...
          <div className="relative bg-zinc-900/40 backdrop-blur-xl rounded-2xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 overflow-y-auto custom-scrollbar">
              <form id="fieldForm" onSubmit={saveField} className="space-y-4">
                <div>
                  <label className="block text-sm font-semibold text-zinc-300 mb-1">Question Title</label>
                  <input required type="text" value={fieldData.label} onChange={e => setFieldData({...fieldData, label: e.target.value})} className="w-full border-zinc-700/50 bg-zinc-950/50 rounded-xl px-4 py-2.5 outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500" placeholder="e.g. What is your name?" />
                </div>
                
                <div>
                  <label className="block text-sm font-semibold text-zinc-300 mb-1">Answer Type</label>
                  <select value={fieldData.type} onChange={e => setFieldData({...fieldData, type: e.target.value})} className="w-full border-zinc-700/50 bg-zinc-950/50 rounded-xl px-4 py-2.5 outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
                ...
                {(fieldData.type === "select" || fieldData.type === "radio") && (
                  <div>
                    <label className="block text-sm font-semibold text-zinc-300 mb-1">Options</label>
                    <textarea required rows={3} value={fieldData.options} onChange={e => setFieldData({...fieldData, options: e.target.value})} placeholder="Option 1, Option 2, Option 3" className="w-full border-zinc-700/50 bg-zinc-950/50 rounded-xl px-4 py-2.5 outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 resize-none" />
                    <p className="text-xs text-zinc-400 mt-1">Separate options with commas.</p>
                  </div>
                )}
                
                {fieldData.type === "relation" && (
                  <div>
                    <label className="block text-sm font-semibold text-zinc-300 mb-1">Relation Target</label>
                    <select 
                      value={(fieldData as any).relationTarget || "Project"} 
                      onChange={e => setFieldData({...fieldData, relationTarget: e.target.value} as any)} 
                      className="w-full border-zinc-700/50 bg-zinc-950/50 rounded-xl px-4 py-2.5 outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
                    >
                ...
                {["text", "textarea", "email", "number"].includes(fieldData.type) && (
                  <div>
                    <label className="block text-sm font-semibold text-zinc-300 mb-1">Placeholder Text (Optional)</label>
                    <input type="text" value={fieldData.placeholder} onChange={e => setFieldData({...fieldData, placeholder: e.target.value})} className="w-full border-zinc-700/50 bg-zinc-950/50 rounded-xl px-4 py-2.5 outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500" />
                  </div>
                )}
                
                <div className="flex items-center justify-between pt-2 border-t border-zinc-800/60 mt-4">
      ...
      {isShareModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-zinc-900/40 backdrop-blur-xl rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="p-6">
              ...
              <div className="space-y-6">
                {/* Link */}
                <div>
                  <label className="block text-sm font-semibold text-zinc-300 mb-2">Direct Link</label>
                  <div className="flex items-center gap-2">
                    <input 
                      type="text" 
                      readOnly 
                      value={publicUrl} 
                      className="flex-1 bg-zinc-950/50 border border-zinc-700/50 rounded-xl px-4 py-2.5 text-sm text-zinc-400 outline-none" 
                    />
                    <button 
                      onClick={() => {
                ...
                {/* Iframe */}
                <div>
                  <label className="block text-sm font-semibold text-zinc-300 mb-2">Embed on Website (Iframe)</label>
