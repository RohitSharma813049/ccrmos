    ...
    <div className="flex gap-6 overflow-x-auto pb-4 h-full min-h-[500px]">
      <DragDropContext onDragEnd={onDragEnd}>
        {columns.map((colId) => {
          ...
          return (
            <div key={colId} className="flex-shrink-0 w-80 flex flex-col bg-gray-50 rounded-2xl border border-gray-200">
              <div className="p-4 border-b border-gray-200 flex justify-between items-center bg-white rounded-t-2xl">
                <h3 className="font-semibold text-gray-800 capitalize">{colId}</h3>
              ...
              <Droppable droppableId={colId}>
                {(provided, snapshot) => (
                  <div
                    ...
                    className={`flex-1 p-4 flex flex-col gap-3 min-h-[150px] transition-colors ${snapshot.isDraggingOver ? 'bg-blue-50/50' : ''}`}
                  >
