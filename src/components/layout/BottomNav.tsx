...
export default function BottomNav({ children }: { children?: React.ReactNode }) {
  ...
  return (
    <>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 w-full bg-white/90 backdrop-blur-md border-t border-gray-200/50 z-50 flex items-center justify-around pb-safe pt-1 shadow-[0_-4px_16px_rgba(0,0,0,0.03)] h-16">
        ...
        <button 
          ...
          className={`flex flex-col items-center justify-center w-[20%] h-full gap-1 transition-all duration-200 ${showMore ? 'text-blue-600' : 'text-gray-500 hover:text-gray-900'}`}
        >
      ...
      {showMore && (
        <div className="md:hidden fixed inset-0 z-40 bg-gray-900/60 backdrop-blur-sm transition-all" onClick={() => setShowMore(false)}>
          <div 
            className="absolute bottom-16 left-0 w-full bg-white rounded-t-3xl shadow-[0_-8px_30px_rgba(0,0,0,0.12)] pb-4 overflow-hidden animate-in slide-in-from-bottom-full duration-300 ease-out"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex flex-col items-center pt-3 pb-2">
              <div className="w-12 h-1.5 bg-gray-200 rounded-full mb-3" />
              <div className="w-full flex justify-between items-center px-6">
                <h3 className="text-xl font-bold text-gray-900 tracking-tight">Main Menu</h3>
                <button onClick={() => setShowMore(false)} className="p-2 bg-gray-100/80 text-gray-600 rounded-full hover:bg-gray-200 active:scale-95 transition-all">
...
function NavItem({ href, label, icon, active }: { href: string; label: string; icon: string; active?: boolean }) {
  return (
    <Link
      href={href}
      className={`flex flex-col items-center justify-center w-[20%] h-full gap-1 transition-all duration-200 active:scale-95 ${
    ...
    >
      <div className={`relative flex items-center justify-center transition-transform duration-200 ${active ? 'scale-110' : ''}`}>
        ...
        {active && <span className="absolute -bottom-2 w-1 h-1 bg-blue-600 rounded-full" />}
      </div>
